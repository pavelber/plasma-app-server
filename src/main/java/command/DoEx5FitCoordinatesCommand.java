...
public class DoEx5FitCoordinatesCommand implements Command
{
...
//		res = a / energy + b / energy / energy + c / Math.pow(energy, 3) + 
//		d / Math.pow(energy, 4) + e / energy * Math.log(energy);
		...
		for(int i = 0; i <= NUMBER_OF_POINTS; ++i)
		{
			...
			if(Double.isInfinite(kB) || Double.isNaN(kB))
			{
           	...
           	double kC = m_dC / Math.pow(xi, 3.0);
			if(Double.isInfinite(kC) || Double.isNaN(kC))
			{
           	...
           	double kD = m_dD / Math.pow(xi, 4.0);
